...
$$
\bar{A} = \exp(A\Delta) \\
\bar{B} = \int_0^\Delta \exp(A\tau) \, d\tau \cdot B = A^{-1}(\bar{A} - I)B \quad \text{(若 \( A \) 可逆)}
$$
...
<div align="center">
  ...
  <figcaption>图 12 S4 和 S6 参数维度对比</figcaption>
  <p></p>
</div>
...
由于我们的参数矩阵是随着输入而变化的，所以不能直接使用卷积操作以通过并行的方式加速训练，只能采用递归计算的方式进行。但是我们仍然可以使用一些并行扫描算法来对训练进行并行加速。

Mamba 模型中的「并行扫描算法」是其高效性的核心之一。它主要解决了 *线性状态空间模型 (SSM)* 在序列建模中的一个瓶颈：**如何在保持线性递推结构的同时，实现高效的并行计算**。

在传统的离散时间状态空间模型中，一个序列的每个状态 $$x_t$$ 都是通过递推计算得出的：

$$
x_t = A x_{t-1} + B u_t
\quad\text{且输出：}\quad y_t = C x_t
$$

这是一个 **递推结构**，只能一步步计算，**不利于并行处理**。

Mamba 利用了 **「线性状态空间模型」的结构性**，并结合一个核心技巧：

> **将递推转化为带权前缀和（prefix scan）操作，使其可以并行计算。**

Mamba 中实际更新的形式是：

$$
x_t = \Delta_t \odot x_{t-1} + B_t \odot u_t
$$

其中：

* $$\Delta_t = \exp(A \cdot \delta_t)$$，是每个位置的衰减系数，具有类似 gating 效果
* $$B_t$$, $$u_t$$: 输入项（可为时间相关）

这是一个 **元素级的加权递推**，可以通过如下方式实现**并行化**：

Mamba 使用了类似于 **[parallel scan](https://en.wikipedia.org/wiki/Prefix_sum)** 算法的方式，将这个递推写成：

$$
x_t = \sum_{s=0}^{t} \left( \prod_{k=s+1}^t \Delta_k \right) \cdot B_s u_s
$$

这就是所谓的 **可乘的卷积核表示**，也叫 “scan over kernel-weighted input”。

这种形式是**乘法-加法结构**，可以用并行前缀积（prefix product） + 前缀和来实现。这个技巧最早被广泛用于 S4/Mega/mamba 等 SSM 模型中。

- 实现并行扫描的三步法（简化理解）

1. **计算衰减因子序列**：
   先计算所有时间步的衰减系数（scan kernel）：
...
2. **计算前缀积**（乘积扫描）
   ...
   $$
   \prod_{k=s+1}^{t} \Delta_k
   $$

   使用并行 scan 实现所有时间步的乘积项。

3. **加权求和输出**：
   最终加权历史输入 $$B_s u_s$$ 得到：

   $$
   x_t = \sum_s \text{weight}_{t,s} \cdot B_s u_s
   $$


- **CUDA 实现与 Triton 加速**

Mamba 实现中还使用了 Triton 和 CUDA kernel 自定义加速该并行 scan 操作，使得推理性能极高：

* 单层 SSM 的前向传播能以 **高效矩阵运算**方式计算
* 相比递推版本速度大幅提高（比 LSTM/Transformer 快数倍）

- 总结：Mamba 的并行扫描特点

| 特性   | 说明                             |
| ---- | ------------------------------ |
| 核心思想 | 将线性递推改写为带权前缀和（scan）            |
| 算法关键 | 前缀乘积 + 加权前缀和                   |
| 实现技术 | einsum + CUDA kernel（或 Triton） |
| 优势   | 同时保留线性记忆结构 + 支持 GPU 并行         |
| 模型表现 | 高速推理，性能优于 Transformer/S4 等     |
...
重点需要关注 $$\bar A, \bar B$$ 两个矩阵的爱因斯坦求和约定运算。关于选择扫描这一块是怎么实现的，我还没有看明白（笨死了！）等后面看看大佬的解读再回过头来写一写……
...
<div align="center">
  <img src="/assets/images/2025/5.23/S6.png" width="600">
...
<div align="center">
  <img src="/assets/images/2025/5.23/MambaBlock.svg" width="500">
...
<div align="center">
  <img src="/assets/images/2025/5.23/mambarnntransformer.svg" width="500">
  <figcaption>图 18 Mamba Block 的构成（端到端）</figcaption>
  <p></p>
</div>

呜啊，看了两天的 mamba 模型还是没能完全理解，对于一些细节上的问题明天再补充吧！
